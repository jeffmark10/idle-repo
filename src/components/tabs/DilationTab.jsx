...
import { 
  ...
  CheckCheck, Table, AlertTriangle, Search, Zap, Layers, Share2,
  Info, Clock, Compass, ShieldAlert, CheckCircle2, Trophy, Star
} from "lucide-react";
...
export default function DilationTab() {
  ...
  // Gera a string oficial no formato Cx;Tx,x,x,x;Mx,x,x,x;Bx,x,x,x
  ...
  const DILATION_TABS = [
    ...
    { id: "guide", label: "2. Roteiro Pré-Árvore (Início até DTP 1)", icon: Compass },
    { id: "unity_prep", label: "3. Rumo à Unidade (DTP 40+ & Unity)", icon: Zap },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ========================================================================= */}
      {/* 0. O QUE É A DILATAÇÃO? (INTRODUÇÃO DA CAMADA 4) */}
      {/* ========================================================================= */}
      {subSection === "overview" && (
        <div className="space-y-6">
          <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-cyan-950/40 via-zinc-900/80 to-zinc-950 border-2 border-cyan-500/30 shadow-2xl space-y-4">
            ...
            <p className="text-xs sm:text-sm text-zinc-300 leading-relaxed max-w-4xl">
              Desbloqueada no Marco 7 de Animais, a <strong>Dilatação</strong> introduz severas penalidades globais à sua corrida. Seu objetivo dentro da Dilatação é empurrar o <strong>Max Score</strong> o mais alto possível, pois ele determina sua <strong>Renda Passiva de Pontos de Dilatação (DP/s)</strong>, que continuará gerando mesmo fora da Dilatação.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-2 font-mono text-xs">
              ...
              <div className="p-3.5 rounded-2xl bg-zinc-950/90 border border-cyan-500/30 space-y-1">
                ...
                <p className="text-zinc-400 font-sans text-[11px] leading-relaxed">
                  • <strong>DU1 (Suavizador):</strong> Reduz todas as penalidades em 0.03 por nível (Prioridade máxima dentro da Dilatação).<br />
                  • <strong>DU2 (Renda DP):</strong> Multiplica sua renda de DP/s em x1.07 por nível (Compre durante runs de EP/SN).
                </p>
              </div>
          ...
          {/* Guia das 9 Melhorias de Dilatação */}
          <div className="p-6 rounded-3xl bg-zinc-900/40 border border-zinc-800 space-y-3 font-sans text-xs">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 font-mono text-xs">
              <div className="p-3 bg-zinc-950 rounded-2xl border border-zinc-800 space-y-1">
                ...
                <p className="text-zinc-400 font-sans text-[11px]">DU1 suaviza as penalidades para empurrar o Score. DU2 multiplica o ganho passivo de DP/s. Mantenha DU2 com cerca de metade do nível de DU1.</p>
              </div>
              <div className="p-3 bg-zinc-950 rounded-2xl border border-zinc-800 space-y-1">
                <strong className="text-emerald-300 block text-xs">DU3 a DU6 (Apoio a Score/Lab)</strong>
                <p className="text-zinc-400 font-sans text-[11px]">Concedem níveis livres de RP, aumentam o expoente comum e geradores. Compre apenas quando custarem cerca de 1/3 do valor de DU1.</p>
              </div>
              <div className="p-3 bg-zinc-950 rounded-2xl border border-zinc-800 space-y-1">
                <strong className="text-purple-300 block text-xs">DU7 a DU9 (Secundários/Finais)</strong>
                <p className="text-zinc-400 font-sans text-[11px]">Aumentam ganho de EP, Ascensão e Poder do Lab. DU7 deve ser comprado logo antes de terminar runs de EP/Supernova.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ========================================================================= */}
      {/* 1. ÁRVORE ESPACIAL & BUILDS (DTP 1 A 48) - LAYOUT ORIGINAL PRESERVADO */}
      {/* ========================================================================= */}
      {subSection === "tree" && (
        <div className="space-y-6">
          {/* Hero Header dos Presets Finais */}
          ...
          {/* Grid Principal: Alocador Espacial e Tabela Pareados em Altura */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
            
            {/* Lado Esquerdo: Alocador Espacial da Árvore */}
            <div className="lg:col-span-7 p-4 sm:p-5 rounded-3xl bg-zinc-900/40 border border-zinc-800 flex flex-col justify-between space-y-3">
              ...
              {/* Barra de Cópia da String da Árvore Atual */}
            ...
            {/* Lado Direito: Tabela Sequencial de Builds (DTP 1 a 40+) */}
            <div className="lg:col-span-5 p-4 sm:p-5 rounded-3xl bg-zinc-900/40 border border-zinc-800 flex flex-col justify-between space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-zinc-800 pb-2.5">
                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                  <Table className="w-4 h-4 text-cyan-400" /> Builds (DTP 1 a 40+)
                </h3>
      ...
      {/* ========================================================================= */}
      {/* 2. ROTEIRO PRÉ-ÁRVORE (INÍCIO ATÉ DTP 1) */}
      {/* ========================================================================= */}
      {subSection === "guide" && (
        <div className="space-y-6">
          <div className="p-4 sm:p-5 rounded-3xl bg-zinc-900/80 border border-cyan-500/30 font-sans text-xs space-y-2 shadow-lg">
            <div className="flex items-center gap-2 font-mono text-cyan-400 font-bold">
              <Compass className="w-4 h-4" />
              <span>Sobre esta Sub-Aba: O Caminho de Progressão da Dilatação Inicial até o 1º DTP</span>
            </div>
            <p className="text-zinc-300 leading-relaxed">
              Esta seção detalha os 4 estágios da Dilatação antes do desbloqueio da Árvore: a primeira corrida curta para gerar renda de DP/s, a obtenção de Estrelas dentro da Dilatação, a conclusão do Desafio EC10 e o acúmulo de 1e27 DP para o DTP 1.
            </p>
          </div>

          {/* As 4 Fases Pré-Árvore */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-sans text-xs">
            ...
            <div className="p-5 rounded-3xl bg-zinc-900/50 border border-zinc-800 space-y-2.5">
              ...
              <p className="text-zinc-400 leading-relaxed">
                • Cada dificuldade do EC10 multiplica sua renda de DP/s (x10, x5, x3, x2.6, x2.5).<br />
                • <strong>EC10-1:</strong> DU1 ~160 (Meta: 1.000.000 Score bruto).<br />
        ...
        <div className="space-y-6">
          <div className="p-4 sm:p-5 rounded-3xl bg-zinc-900/80 border border-purple-500/30 font-sans text-xs space-y-2 shadow-lg">
            <div className="flex items-center gap-2 font-mono text-purple-400 font-bold">
              ...
              <span>Sobre esta Sub-Aba: Preparação para a 5ª Camada (Unidade)</span>
            </div>
            <p className="text-zinc-300 leading-relaxed">
              A <strong>Unidade (Unity)</strong> é a próxima grande camada de prestígio, desbloqueada ao atingir <strong>1.08e2466 EP</strong>. Ela reseta todas as camadas anteriores em troca de Zodíacos, Planetas e Minerais. Siga as recomendações abaixo para não realizar o reset prematuramente.
            </p>
          </div>

          {/* Checklist para Unidade */}
          <div className="p-6 rounded-3xl bg-gradient-to-br from-purple-950/40 via-zinc-900/70 to-zinc-950 border-2 border-purple-500/30 shadow-2xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 font-sans text-xs">
              ...
              <div className="p-3.5 bg-zinc-950 rounded-2xl border border-zinc-800 space-y-1">
                <strong className="text-white block font-mono">✓ Supernova 154+ & 370k+ AP</strong>
                <p className="text-zinc-400">Animal Points não gastos persistem através da Unidade e aumentam a renda inicial de DP.</p>
              </div>
          ...
          {/* Dicas Estruturais do DTP 16 e DTP 40 */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 font-sans text-xs">
