...
export default function MacrosTab() {
  ...
  const filteredMacros = useMemo(() => {
    return ALL_MACROS.filter(m => {
      const matchCat = 
        ...
        selectedCategory === "favorites" ? favoriteMacros.includes(m.id) :
  ...
  }, [selectedCategory, searchTerm, favoriteMacros]);
    ...
    <div className="space-y-6">
      <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-emerald-950/40 via-zinc-900/70 to-zinc-950 border-2 border-emerald-500/30 shadow-2xl space-y-4">
        ...
        <p className="text-xs sm:text-sm text-zinc-300 leading-relaxed max-w-4xl">
          Clique no menu de qualquer macro para expandir e visualizar o script completo. Clique na estrela ⭐ para fixar as macros que você mais usa.
        </p>
          ...
          {filteredMacros.length === 0 ? (
            ...
            <div className="space-y-3">
              {filteredMacros.map((macro) => {
                ...
                const isFavorite = favoriteMacros.includes(macro.id);
