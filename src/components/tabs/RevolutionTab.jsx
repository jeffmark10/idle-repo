...
import { 
  ...
  Flag, Keyboard, Edit3, Zap, BookOpen, AlertCircle
} from "lucide-react";
...
export default function RevolutionTab() {
  ...
  // Sincroniza estado local ao resetar ou receber alteração externa
  useEffect(() => {
    setPromoLevelInput(String(promoLevel));
  }, [promoLevel]);
  ...
  // Ranking com os dados reais de gameState
  const calculatedLbScore = useMemo(() => {
    const ipVal = parseIncrementalNumber(gameState.stats.ip).value;
    const epVal = parseIncrementalNumber(gameState.stats.ep).value;
    const dpVal = parseIncrementalNumber(gameState.stats.dp).value;

    return calculateLeaderboardScore({
      maxScore: parsedScore.value,
      maxExponent: pExp,
      maxInfinites: gameState.stats.eternities > 0 ? 100 : 0,
      maxIP: ipVal,
      maxChallenges: 0,
      maxStars: gameState.stats.stars,
      eternities: gameState.stats.eternities,
      maxEP: epVal,
      maxAnimals: 0,
      maxLabLevel: gameState.stats.lab.base + gameState.stats.lab.mult + gameState.stats.lab.power,
      maxSupernova: gameState.stats.supernovas,
      maxDP: dpVal,
      maxDTP: 0,
      maxUnities: 0,
      maxZodiacLevel: 0,
  ...
  }, [parsedScore.value, pExp, gameState.stats]);
              ...
              <div className="p-4 rounded-2xl bg-zinc-950/80 border border-zinc-800 space-y-2.5">
                ...
                <input
                  ...
                  className={`w-full bg-zinc-900 border rounded-xl px-3 py-2 text-sm text-white font-mono focus:outline-none ${
                    parsedScore.isValid ? "border-orange-500/40 focus:border-orange-400" : "border-red-500 bg-red-950/10"
                  }`}
                />
                ...
                {!parsedScore.isValid && (
                  <div className="flex items-center gap-1.5 text-red-400 text-xs font-sans">
                    ...
                    <span>Valor inválido. Use números comuns ou notação científica (ex: 1000, 1.5e12, 500k).</span>
                  </div>
                )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <div className="lg:col-span-6 p-6 rounded-3xl bg-zinc-900/40 border border-zinc-800 space-y-4">
            ...
            <div className="p-4 rounded-2xl bg-zinc-950 border border-zinc-800 text-[11px] text-zinc-400 space-y-1.5 leading-relaxed font-mono">
              ...
              <div>• <strong>Eternidades & IP:</strong> Baseados nas estatísticas atuais</div>
            </div>
          </div>
        </div>
      )}
