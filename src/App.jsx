...
import { 
  ...
  Sparkles, Terminal, Layers, Trash2
} from "lucide-react";
...
const MAIN_LAYERS = [
  ...
  { id: "unity", label: "Unidade", shortLabel: "Unity", icon: Layers, color: "text-amber-400", activeBg: "bg-amber-600 text-white" },
...
export default function App() {
  ...
  const activeTab = gameState.profile.currentTab || "revolution";

  const handleReset = () => {
    if (window.confirm("Isso apagará todo o progresso salvo do guia (checklists, árvore de Dilatação e favoritos). Deseja continuar?")) {
      resetAllData();
    }
  };
      ...
      <header className="sticky top-0 z-40 bg-zinc-950/90 backdrop-blur-md border-b border-zinc-800/80 px-3 py-2.5 sm:px-6">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-2">
          ...
          <nav className="hidden md:flex items-center gap-1 bg-zinc-900/60 p-1 rounded-2xl border border-zinc-800 font-mono text-xs">
            {MAIN_LAYERS.map((layer) => {
              ...
              return (
                <button
                  key={layer.id}
                  onClick={() => setCurrentTab(layer.id)}
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleReset}
              title="Resetar todo o progresso salvo"
              aria-label="Resetar progresso salvo"
              className="p-1.5 rounded-lg bg-zinc-900 border border-zinc-800 text-zinc-500 hover:text-red-400 hover:border-red-500/40 transition-colors"
            >
